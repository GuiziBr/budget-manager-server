...
export class BudgetPeriodService {
	...
	async update(id: string, dto: UpdateBudgetPeriodDTO): Promise<BudgetPeriod> {
		this.logger.debug(`Updating budget period with id: ${id}`)
		try {
			await this.findById(id)
			return await this.budgetPeriodRepository.update(id, dto)
		} catch (error) {
			if (error instanceof HttpException) throw error
			this.logger.error(`Failed to update budget period with id: ${id}`, error)
			throw new InternalServerErrorException()
		}
	}
